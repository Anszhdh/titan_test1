...
const updateShipping = async () => {
    await Inertia.put(`/admin/subscriptions/${props.subscription.id}/update-shipping`, {
...
onMounted(() => {
    // Check if subscription already has shipping details
    if (props.subscription.shipping) {
        shippingType.value = props.subscription.shipping.shipping_type || '';
        trackingNumber.value = props.subscription.shipping.tracking_number || '';
    }
});

watch(() => props.subscription, (newSubscription) => {
    // Update fields if Subscription changes
    if (newSubscription.shipping) {
        shippingType.value = newSubscription.shipping.shipping_type || '';
        trackingNumber.value = newSubscription.shipping.tracking_number || '';
    } else {
        shippingType.value = '';
