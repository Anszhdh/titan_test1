...
// Watch for changes in props to update recommendations and products
watch(() => props.value, (newProps) => {
    recommendations.value = newProps.recommendations || [];
    products.value = newProps.products || [];
});
</script>
    ...
    <FooterLayout>
        <AuthenticatedLayout>
            <div class="container mx-auto px-4 py-8 bg-orange-100/5">
                ...
                <div v-if="recommendations.length === 0" class="text-center mb-8">
                    <p>No recommendations available at the moment.</p>
                </div>
                <div v-else>
                    <div v-for="(recommendation, index) in recommendations" :key="recommendation.id || index" class="text-center mb-8">
                        ...
                        <div v-if="products[index]" class="bg-white shadow-md rounded-lg p-4">
                            <p class="text-lg mb-4">Price: RM {{ products[index]?.price || 'N/A' }}</p>
                            <div class="flex justify-center mb-4">
                                <img :src="products[index]?.image_url || '/placeholder.jpg'" alt="Product Image" class="h-80 w-80 object-cover rounded-md border-2 border-gray-200" />
                            </div>
                            <div class="flex justify-center mb-4">
                                <button
                                    v-for="frequency in ['Weekly', 'Bi-Weekly', 'Monthly']"
                                    :key="frequency"
                                    :class="{'bg-brown-500 text-white': selectedFrequency === frequency, 'bg-yellow-950 text-white hover:bg-yellow-900': selectedFrequency !== frequency}"
