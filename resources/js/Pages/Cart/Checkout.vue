...
function checkout() {
  Inertia.post('/checkout/process', { totalPrice: totalPrice.value }, {
    onSuccess: (response) => {
      Inertia.visit(`/checkout/success/${response.order_id}`);
    },
    onError: (error) => {
      console.error('Error during checkout:', error);
      alert('Error: Unable to complete checkout. Please try again later.');
    }
  });
}
</script>
